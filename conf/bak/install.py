...
# web配置文件修改
    for web_app in web_all:
        if web_app == 'openservice':
            run("bash /tmp/config_c.sh {web_app} {web_path} {l_redis} {c_redis} {kafka} {zookeeper} {mysql}".format(web_app=web_app,
                web_path=web_path+"/api/"+web_app, l_redis=app_ip.get('config','local_redis'),c_redis=app_ip.get('config','cluse_redis'),
                kafka=app_ip.get('config','kafka'),zookeeper=app_ip.get('config','zookeeper'),mysql=app_ip.get('config','mysql')))
        else:
            run("bash /tmp/config_c.sh {web_app} {web_path} {l_redis} {c_redis} {kafka} {zookeeper} {mysql}".format(web_app=web_app,
